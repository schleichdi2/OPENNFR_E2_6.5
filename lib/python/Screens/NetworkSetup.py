...
from os import system, path as os_path, system, remove, unlink, rename, chmod, access, X_OK
...
class NetworkAfp(Screen):
	...
	def checkNetworkStateFinished(self, result, retval,extra_args=None):
		...
		elif ('Collected errors:' or 'wget returned 1' or 'wget returned 8' or 'wget returned 255' or '404 Not Found') in result:
...
class NetworkSamba(Screen):
	def __init__(self, session):
		...
		self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'ok': self.close, 'back': self.close, 'red': self.UninstallCheck, 'green': self.SambaStartStop, 'blue': self.Sambashowlog, 'yellow': self.Sambaedit})
...
class InetdRecovery(Screen, ConfigListScreen):
	...
	def keyBlue(self):
		...
		self.session.open(MessageBox, _("Successfully restored /etc/inetd.conf!"), type = MessageBox.TYPE_INFO,timeout = 5)
		self.close()
